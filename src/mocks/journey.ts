import type { DriverHome, Driver, DriverRouteSnapshot, Trip, TripStatus } from "@/types";

import { ApiError, delay } from "./latency";
import { DRIVER_REWARD } from "./performance";
...
/** Recorte geográfico fictício da rota Duque de Caxias → Juiz de Fora. */
const ACTIVE_ROUTE = {
  origin: { latitude: -22.7856, longitude: -43.3117 },
  destination: { latitude: -21.7622, longitude: -43.3434 },
  currentPosition: { latitude: -22.1155, longitude: -43.2054 },
  path: [
    { latitude: -22.7856, longitude: -43.3117 },
    { latitude: -22.6712, longitude: -43.2835 },
    { latitude: -22.505, longitude: -43.178 },
    { latitude: -22.3102, longitude: -43.1539 },
    { latitude: -22.1155, longitude: -43.2054 },
    { latitude: -21.9584, longitude: -43.2758 },
    { latitude: -21.7622, longitude: -43.3434 },
  ],
  speedKph: 74,
  etaMinutes: 118,
...
/* Viagem de demonstração usada nos fluxos do aplicativo. */
const state = {
  /* A viagem de demonstração já está em trânsito, portanto o checklist foi concluído. */
  checklistSentToday: true,
  blocked: false,
...
export async function mockDriverHome(): Promise<DriverHome> {
  ...
  return {
    ...
    ...(currentTrip ? { route: { ...ACTIVE_ROUTE, updatedAt: new Date().toISOString() } } : {}),
